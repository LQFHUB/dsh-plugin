...
/**
* describe-image 工具的视觉 HTTP 客户端：加载一张图片（本地路径、http(s)
* URL 或已存附件引用），按配置的协议风格（chat-completions 或 responses）
* 构造端点请求，读回单个文本答案——带短生命周期、容量受限的语义缓存，
* 相同图片与指令的重复调用避免二次往返。响应体与错误摘要在信任任何字节
* 之前先截断。
* @module dsh-describe-image/vision
*/
