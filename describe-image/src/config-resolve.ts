/**
 * describe-image 工具的配置与凭证事实。持有校验后的 ResolvedConfig 快照
 * （默认值、上限与端点事实）、API-key 解析接缝，以及同时充当插件设置卡
 * schema 的 schemastery section。与工具注册和视觉 HTTP 客户端分离，保持
 * 单一职责。
 * @module dsh-describe-image/config
 */
