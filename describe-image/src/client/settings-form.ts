/**
 * 插件设置卡背后的暂存表单模型（由参考包全家桶 shared slice 转写）。卡片
 * 暂存用户输入，只在保存时写入——设置写入是持久的、修订栅栏保护的文档
 * 变更，因此暂存保证屏幕上的内容与保存将存储的内容完全一致。镜像官方
 * ui-plugin-config 卡片存储模式的共享切片。
 * @module dsh-describe-image/client/settings-form
 */
