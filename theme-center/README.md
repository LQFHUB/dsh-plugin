...
> 个人主题插件（一体化）：新增一张「主题」卡片（**dsh 0.1.6+**：Plugins 面板 → 插件详情页；**≤0.1.5**：设置 > 插件配置 TAB），顶部 **主题 / 外观** 双 Tab——
...
- **dsh 0.1.6+**：Plugins 面板（左侧）→ 已安装列表点「主题」→ 详情页双 Tab 表单（`plugins.bundle.config`）
- **dsh ≤0.1.5**：设置（侧边栏底部）→ 插件配置 → 「主题」折叠卡片（`settings.plugin.item`）
...
1. 打开「主题」卡片（dsh 0.1.6+：Plugins 面板 → 点「主题」进详情页；≤0.1.5：设置 → 插件配置 → 主题）
