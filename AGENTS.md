...
<details>
<summary>📜 变更记录（共 13 条，点击展开，最新在最上面）</summary>
...
- **服务重启前处理**：111 上 dsh web 为 systemd 服务 `dsh-web.service`（`Restart=on-failure`，崩溃自动拉起）。重启服务前必须先收尾：① 收集/终止后台任务与子代理；② 提交未提交的 git 变更；③ 未完成目标用 goal 记录。会话历史持久化于 `/root/.dsh/sessions/`，重启后恢复原会话即可继续（goal 用 resume）。
