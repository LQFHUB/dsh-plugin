...
- 备注：**112 部署验证通过**（2026-09-06，npm pack tarball + file: 引用 + pnpm install + 重启）——真实 ask_user_question 弹提问卡片时探针捕获 ding 合成音（基频 698.46…与参数完全一致）、console 0 错误。⚠️ 112 上 notify-sound 现为 file: 本地 tarball 引用（`/root/dsh-notify-sound-0.1.3.tgz`），**勿删**；回滚：specifier 恢复 `^0.1.2` + pnpm install + 重启。待 npm 发布 v0.1.3 + 111 部署（均需用户确认）
