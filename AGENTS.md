...
| `describe-image/` | 图像理解工具（视觉模型描述图片，configured 模式复用已配置模型 + 「图像理解」设置卡） | 112 验证通过；111 待用户确认部署 |
| `right-panel/` | 右侧面板（文件树/多格式预览/SCM 变更面板，语法高亮，harbor/trading 皮肤适配层） | 112 验证通过；111 待用户确认部署 |
