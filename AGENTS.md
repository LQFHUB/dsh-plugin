...
<details>
<summary>📜 变更记录（共 14 条，点击展开，最新在最上面）</summary>
...
- **服务重启前处理**：111 上 dsh web 为 systemd 服务 `dsh-web.service`（`Restart=on-failure`，崩溃自动拉起）。重启服务前先收尾：① 收集/终止后台任务与子代理；② 未完成目标如需继续，用 goal 记录。**重启时无需专门提交 git 变更**；重启后由用户自行刷新浏览器恢复会话。
