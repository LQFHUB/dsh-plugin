...
### 2026-09-23 111（正式机）升级 dsh 0.1.5-rc.1 → 0.1.7-rc.1（经用户确认：只升级 dsh，插件维持现状）

- 变更内容：用户要求「更新111」，经询问确认范围为**只升级 dsh**（插件不动）。① **备份**：`/root/.dsh/profiles/web/{package.json,pnpm-lock.yaml,cordis.patch.yml}`、`/root/.dsh/settings.yaml`（5848B）、sessions 打包（均 `*.bak-pre-017rc1-20260923_231511`；`/root/dsh-sessions-bak-pre-017rc1-20260923_231511.tgz` 177MB / 8 会话）；② **升级** `npm install -g @deepseek-ai/dsh@0.1.7-rc.1`（15s，+81 / -88 / ~433 包；新进程 `dsh --version` = 0.1.7-rc.1）；③ **重启前预检**（用新代码合成 111 插件树）：`dsh --profile web --dump-config` **exit 0、stderr 空**，5 个插件（dsh-global-rules / dsh-notify-sound / dshmarket / @liustack/modsearch / @npm-liqingfeng/dsh-web-lan）全数入树，web-lan 补丁层已应用（`patched by @npm-liqingfeng/dsh-web-lan`）；④ **双重启改用延迟 detach 触发**：脚本 `/root/dsh-upgrade-restart.sh`、日志 `/root/dsh-upgrade-restart.log`（首启 web-lan REPAIRED 写盘、二启 PATCHED 加载）——首次排程 45s 判定过短（会打断记录与汇报）已撤销，改由本回合末尾重新排程。
- 涉及路径：111 `/usr/local/lib/node_modules/@deepseek-ai/dsh`、`/root/.dsh/profiles/web/*`（备份 `*.bak-pre-017rc1-20260923_231511`）、`/root/.dsh/settings.yaml`（备份同上）、`/root/dsh-sessions-bak-pre-017rc1-20260923_231511.tgz`、`/root/dsh-upgrade-restart.sh`；本目录无文件变更
- 备注：⚠️ **本回合无法完成后重启验证**（重启会终止 agent 所在进程）——重启后需确认：服务 active、`dsh --version` = 0.1.7-rc.1、插件树加载、LAN 免 token 303+Set-Cookie、模型调用；`settings.yaml` 将由官方 `importLegacyDocument()` 迁移进 `cordis.patch.yml`（官方段迁入、被拒段留在 `settings.yaml.imported`）；**会话格式 V2→V3 不可降级**。**插件在新版下的预期**（用户选择不更新插件）：notify-sound 0.1.5（link、旧代码）配置卡将不可编辑（`installSection` 已移除，同 112 现象）、modsearch fork 的 settings namespace 注册失败（工具与 `/modsearch/config` 路由正常）、dshmarket 1.47.0 的 `settings.register` 同样静默降级（插件本身应正常）。⚠️ 111 服务无 stdout 重定向，**日志在 `journalctl -u dsh-web`**（不是 `/root/dsh-web.log`）。

### 2026-09-23 112 卸载 theme-center 插件（插件数 7 → 6）

- 变更内容：用户要求「theme-center 插件直接卸载」（此前已确认 theme-center 不做 0.1.7 settings 适配）。① 备份 `profiles/web/{package.json,pnpm-lock.yaml}`（`*.bak-pre-rm-tc-20260923_230447`）；② `dsh plugin --profile web remove @npm-liqingfeng/dsh-theme-center`（467ms）——dependencies 7→6、`dsh.profile.bundles` 10→9、`node_modules/@npm-liqingfeng/dsh-theme-center` 目录已移除；③ 重启 `dsh-web.service`。
- 涉及路径：112 `/root/.dsh/profiles/web/{package.json,pnpm-lock.yaml}`（备份 `*.bak-pre-rm-tc-20260923_230447`）、`/root/.dsh/profiles/web/node_modules/@npm-liqingfeng/dsh-theme-center`（已移除）；本目录无文件变更
- 备注：**验证全过**——服务 active、NRestarts=0、`dsh --profile web --dump-config` exit 0、插件树中 theme-center 已消失（剩 6 个第三方插件 + 官方）、Plugins 面板 Installed **6** 且无 theme-center、页面 `body` 属性只剩 `data-dsh-notify-sound`（`data-dsh-theme-center` 已收回）、**console 0 错误 0 警告**（此前每 15s 的 `/theme-center/settings` 404 轮询噪音随之消失）、web-lan 补丁 `PATCHED` 完好、dshmarket 1.58.0 与 notify-sound 0.1.6 不受影响。**保留物**（均未删）：tarball `/root/npm-liqingfeng-dsh-theme-center-0.5.7.tgz`（862KB）、`/root/.dsh/settings.yaml.imported` 中的 theme-center 配置段（12 字段）、本仓库 `theme-center/` 源码目录。回滚：`dsh plugin --profile web add file:/root/npm-liqingfeng-dsh-theme-center-0.5.7.tgz`（或 specifier 恢复 + `pnpm install`）+ 重启。⚠️ **更正（2026-09-23 复核）**：111 上**并未安装 theme-center**——profile 的 `dependencies`、`dsh.profile.bundles`、`cordis.patch.yml` 均无引用，`dsh --profile web --dump-config` 0 命中；`/root/.dsh/external/theme-center` 仅是遗留源码目录，无需卸载（此前记录误称其为「link 安装」，特此更正）。

### 2026-09-23 dshmarket 更新 1.48.0 → 1.58.0 + LAN 信任修复（LAN 写操作 403 已修复）

- 变更内容：用户要求「把插件市场插件更新到最新版本」。① 备份 `profiles/web/{package.json,pnpm-lock.yaml}`（`*.bak-pre-market-158-20260923_225340`）；② specifier `1.48.0` → `1.58.0`（npm latest）+ `pnpm install`（7.7s，+1 包；peer 告警 `@deepseek-ai/dsh-settings ^0.1.0-rc.7 || ^0.1.1-rc.2 || ^0.1.2-alpha.2` 不匹配 0.1.7，属 semver 预发布规则、非运行失败）；③ 重启 `dsh-web.service`；④ **实测发现 1.58.0 引入 LAN 访问回归**：`lib/http.js` 新增 `loopbackAuthority(host)` 防 DNS rebinding 检查（上游 #678），`sameOrigin()` 对非 loopback Host 直接 403——`lib/routes.js` 中 **27 处**受保护路由（`/install`、`/favorite`、`/channel`、`/groups`、`/bundle-order`、`/rollback`、`/delete-snapshot` 等写操作）在 LAN 访问下全部 403 `untrusted origin`；1.48.0 旧实现仅 `new URL(origin).host === host`（LAN 下通过，已解包对比确认）；⑤ **按用户选择修复 LAN 信任**（保持 1.58.0）：patch `lib/http.js`——新增 `setTrustedAuthoritiesSource()` / `matchesTrustedAuthority()`，`loopbackAuthority()` 在 loopback 之外**额外放行官方 `connection.trustedHosts` 中的 authority**（匹配语义复刻 modsearch 先例：无端口条目按 hostname 匹配任意端口、带端口条目精确匹配）；`lib/index.js` 的 `apply()` 内 `ctx.inject(['connection'], ...)` 注入 `() => connCtx.connection?.trustedHosts`；⑥ 补丁固化到 112 `/root/dshmarket-lanfix/`（`http.js` + `index.js` + `README.md`，含重新应用/验证/回滚步骤），原文件备份 `lib/*.bak-lanfix-20260923_230230`。
- 涉及路径：112 `/root/.dsh/profiles/web/{package.json,pnpm-lock.yaml}`（备份 `*.bak-pre-market-158-20260923_225340`）、`/root/.dsh/profiles/web/node_modules/dshmarket`；本目录无文件变更
- 备注：**LAN（192.168.31.112:3080）实测**——✅ 浏览正常：设置面板出现「Plugin Market」入口（`settings.section` 槽在 0.1.7 仍保留），面板渲染 v1.58.0 + Discover/Themes/Favorites/Installed 标签 + 4.2k 插件目录与真实条目（DSH-better-sidebar ★3.7k）、`/dsh-market/check` 200、`/dsh-market/api/v1/capabilities` 200、插件 `1 total · 1 running`；❌ **写操作 403**：`POST /dsh-market/{install,favorite,channel}` 带 LAN Origin 实测 403（LAN 下无法安装/收藏/切换渠道，只能浏览）。⚠️ 日志另现 `skipping profile bundle "@deepseek-ai/dsh-base" / "@deepseek-ai/dsh-experimental-agent-team-profile"`（首次出现于本次 pnpm install 后），但 `dsh --profile web --dump-config` exit 0、官方插件树完整（35 个 tool 条目）、页面与 7 插件均正常，判定为解析路径回退警告、无功能影响。**修复后验证**：LAN 写操作 `POST /dsh-market/{favorite,channel,install}` 由 403 → **400**（已过围栏、进入业务校验）；**安全回归**——`Origin: http://evil.com` 仍 403、非受信 Host（192.168.31.99）仍 403；浏览器端到端：市场面板点收藏 → `Favorites (1)`、取消后回 0，console 无 dsh-market 403（仅剩未适配的 theme-center 404）。**未提交代码**（第三方插件，仅 112 环境变更）。回滚：specifier 恢复 `1.48.0` + `pnpm install` + 重启，或按 `/root/dshmarket-lanfix/README.md` 还原两个 lib 文件。⚠️ 补丁位于 `node_modules` 内，**dshmarket 升级会被覆盖**，需按 README 重新应用。
