...
`;
				...
				const sidebarOf = () => {
					...
					const btn = document.querySelector('button[aria-label="Collapse sidebar"], button[aria-label="Open sidebar"], button[aria-label="Expand sidebar"], button[aria-label="收起侧边栏"], button[aria-label="打开侧边栏"], button[aria-label="展开侧边栏"]');
					if (btn !== null) {
						const c = containerOf(btn.parentElement);
						if (c !== null) return c;
					}
					return document.querySelector('[data-side="sidebar"]');
				};
				const position = () => {
					const flow = flowOf();
					if (flow === null) return;
