...
/**
 * 同源护栏（本仓库适配，替代上游 loopback-only 护栏）：浏览器在跨站页面
 * 上发起的请求一律拒绝——一个恶意网页不能通过 CSRF 驱动本插件的 fs/git
 * 路由。无 Sec-Fetch-Site / Origin 头的请求（curl 等）放行（本地单用户
 * 工具）。与 theme-center 的护栏语义一致：既挡跨站，又允许经局域网 IP
 * 访问 GUI（上游 loopback 护栏会把非 localhost 访问全部 403）。
 */
...
/** Write the shared cross-site rejection. */
