...
export function apply(ctx, config = {}) {
  ...
  //
  // webServer and connection both exist only under the web profile, and this
  // cordis has no optional-inject form, so both halves ride scoped ctx.inject
  // calls: the closure runs where the services exist and never runs where they
  // do not. The card's route fence follows the /api fence's trustedHosts
  // grant, so the connection service (which owns that list) rides the same
  // inject. A headless profile keeps the plugin's tools and search provider
  // untouched.
...
/**
 * Whether a non-loopback authority is trusted, matching the /api fence's
 * trustedHosts grant (client-connection): a port-less entry matches the
 * hostname on any port (the shape dsh derives for IP-literal LAN serving),
 * an exact host:port entry matches that authority only.
 */
function isTrustedAuthorityHost(hostUrl, trustedHosts) {
 ...
 * must name a loopback authority — or one of the deployment's trusted
 * authorities, the same list client-connection derives from an all-interface
 * bind and admits through --trusted-host: a rebound page reaches this socket
 * carrying its own domain there. Origin and Sec-Fetch-Site then rule out a
 * cross-site page on the machine itself. A read is refused the same way as a
 * write, though it carries no key: nothing about editing engine settings
 * wants a wider door.
 */
function isTrustedRequest(req, trustedHosts) {
  ...
  if (!isLoopbackHost(hostUrl.hostname) && !isTrustedAuthorityHost(hostUrl, trustedHosts)) {
    return false;
  }
...
function registerConfigRoute(ctx) {
  // The /api fence owns the deployment's trusted authorities
  // (all-interface-derived LAN IP literals plus any --trusted-host entries);
  // the card's fence follows the same list, so a LAN-hosted page reaches its
  // own route exactly when it reaches the rest of the web UI.
  const trustedHosts = Array.isArray(ctx.connection?.trustedHosts)
    ? ctx.connection.trustedHosts
    : [];
