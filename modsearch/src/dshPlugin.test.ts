...
/** A host that offers webServer, connection, and settings on scoped injects,
 * like dsh's web profile. `trustedHosts` feeds the connection service the
 * /api fence would derive from an all-interface bind (fork: LAN fix). */
function house(trustedHosts: string[] = []) {
 ...
 * `trustedHosts` are the authorities the connection service derives for an
 * all-interface bind (fork: LAN fix). */
async function withConfig(
  ...
  try {
    ...
    const stage = house(trustedHosts);
...
describe('dsh settings card route', () => {
  ...
  it('trusts a derived LAN authority the connection fence would accept (fork: LAN fix)', async () => {
    // A LAN deployment reaches the card through the machine's own IP literal;
    // the /api fence admits it via client-connection's trustedHosts, so the
    // card's fence must follow: the same Host that 403s before lands 200 now.
    await withConfig(
      { engine: '' },
      async (handler) => {
        const viaLan = await callRoute(handler, {
          method: 'GET',
          url: '/modsearch/config',
          headers: { host: '192.168.31.111:3080' },
        });
    ...
    // A non-loopback authority the fence has not been told to trust stays out.
    await withConfig(
      { engine: '' },
      async (handler) => {
        const other = await callRoute(handler, {
          method: 'GET',
          url: '/modsearch/config',
  ...
  it('serves the settings namespace the card is keyed by', async () => {
    await withConfig({ engine: '' }, async (_handler, _file, stage) => {
      ...
      // The card's fence follows the /api fence's trustedHosts grant, so the
      // route injects connection alongside webServer (fork: LAN fix).
      expect(stage.injected).toContainEqual(['webServer', 'connection']);
